...
class Page {
    ...
    public function show() {
        ...
        if ($downRow <= $this->totalPages){
            $downPage   =   "<a class='down' data-role='button' href='".str_replace('__PAGE__',$downRow,$url)."'>".$this->config['next']."</a>";
        }else{
            $downPage   =   '';
        }
        ...
        if ($this->totalPages > $this->rollPage) {
            if($this->nowPage - $middle < 1){
                ...
                $theFirst   =   "<a class='page-theFirst' href='".str_replace('__PAGE__',1,$url)."' >".$this->config['first']."</a>";
            }
            if($this->nowPage + $middle > $this->totalPages){
                $theEnd     =   '';
            }else{
                $theEndRow  =   $this->totalPages;
                $theEnd     =   "<a class='page-theEnd' href='".str_replace('__PAGE__',$theEndRow,$url)."' >".$this->config['last']."</a>";
            }
        }
        ...
        if ($this->totalPages != 1) {
            ...
            for ($page = $start; $page <= $end; $page++) {
                if ($page != $this->nowPage) {
                    $linkPage .= " <a class='page-nocurrent' href='".str_replace('__PAGE__',$page,$url)."'>&nbsp;".$page."&nbsp;</a>";
                } else {
                    $linkPage .= " <span class='currentcc'>".$page."</span>";
                }
            }
        }
        $pageStr     =   str_replace(
    ...
    public function fshow() {
        ...
        if ($this->totalPages > $this->rollPage) {
            if($this->nowPage - $middle < 1){
                ...
                $theFirst   =   "<a class='page-First' href='".str_replace('__PAGE__',1,$url)."' >1</a> <i>...</i>";
            }
            if($this->nowPage + $middle > $this->totalPages){
                $theEnd     =   '';
            }else{
                $theEndRow  =   $this->totalPages;
                $theEnd     =   "<i>...</i> <a class='page-EndRow' href='".str_replace('__PAGE__',$theEndRow,$url)."' >".$theEndRow."</a>";
            }
        }
