...
class PublicAction extends Action {
    ...
    public function main() {
        ...
        //浏览量
        $tjnum=$tjdate->where($tjmap)->getField('create_num');
        $tjnum1=$tjdate->where($tjmap1)->getField('create_num');
        $tjnum2=$tjdate->where($tjmap2)->sum('create_num');
        ...
        //评论信息
//        $Comment=D('Comment');
//        $ddnum=$Comment->where("from_unixtime(create_time,'%Y-%m-%d')=CURDATE()")->count();
//        $ddnum1=$Comment->where("from_unixtime(create_time,'%Y%m%d')=CURDATE()-1")->count();
//        $ddnum2=$Comment->where("from_unixtime(create_time,'%Y-%m')=DATE_FORMAT(NOW(),'%Y-%m')")->count();
//        
        //浏览排行榜
//        $tjurl=D('Tjurl');
//        $list=$tjurl->order('create_num desc')->limit(10)->select();
    ...
    public function checkLogin() {
        ...
        if(false == $authInfo) {
            ...
            $_SESSION['lynum2']=$lynum2;
            
            //判断使用官方地址
            import('@.ORG.Net.Http');
            $result=Http::GetHttpStatusCode('http://www.djmsm.com');
            if($result==200){
                session('yufu5url','http://www.djmsm.com');
            }else{
                session('yufu5url','http://www.djmsm.com');
            }
