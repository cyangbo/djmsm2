                ...
                if(FALSE!==$list){
                    foreach ($list as $value) {
                        if(isset($value['thumb'])){
                            $path=explode('#', $value['thumb']);
                            foreach ($path as $pathvalue) {
                                $this->delthumb1($pathvalue);
                            }
                        }
                    }
                }
...
//            $this->assign("jumpUrl", $this->getReturnUrl());
    ...
    public function delthumb(){
        ...
        $set=D('Set')->find();
        $thumbSuffix= explode(',', $set['thumbSuffix']);//后缀
        
        //后缀
        foreach ($thumbSuffix as $value) {
            $filename1=substr($filename,0,strrpos($filename, '.')).$value.'.'.$fielextension;
            $src = '../Uploads'.$dir.'/'.$filename1;
    ...
    //根据路径批量删除图片
    public function delthumb1($path){
    ...
    public function _upload(){
        ...
        if(!empty($_FILES))
        {
            ...
            //设置上传文件类型
            $upload->allowExts = explode(',', 'jpg,gif,png,jpeg,bmp,zip,rar');
            //设置附件上传目录
            $y = date('Y',time());
            $m = date('m',time());
            $d = date('d',time());
...
//                            }
    ...
    public function _uploadthumb(){
        ...
        if(!empty($_FILES)){
            ...
            $isthumb=$set['isthumb'];
            $thumbPrefix=$set['thumbPrefix'];//前缀
            $thumbSuffix=$set['thumbSuffix'];//后缀
            ...
            if (!$upload->upload()) {
                ...
                $uploadList = $upload->getUploadFileInfo();
                foreach ($uploadList as $key => $value) {
                    $_POST['thumb'][]='/'.$y.'/'.$m.'/'.$d.'/'.$value['savename'];
    ...
    public function getproinfo() {
        ...
        $mallid=  $this->_get('mid');
        $url=$this->_get('url');
        $isdownimg=  $this->_get('isdownimg');
        
        import('@.ORG.Autogetinfo');
        $agi= new Autogetinfo($mallid, $url);
        $arr=$agi->getInfo();
        
        if($isdownimg=='true'){
            //下载网络图片到服务器端
            $imglist=downImages($arr['webimglist']);
            $arr['imglist']=$imglist;
        }else{
            $arr['imglist']='';
        }
